...
# Disable TensorFlow 2.x eager execution to use TF1.x style code
...
def train_nn(train_data, iterations=1000):
    ...
    try:
        saver = tf.compat.v1.train.import_meta_graph(nn_predictor.META)

        saver.restore(sess, check_point)
        X_tf = sess.graph.get_tensor_by_name('X:0')
        y_tf = sess.graph.get_tensor_by_name('y:0')
        training_op_tf = sess.graph.get_operation_by_name('training_op')
        loss_tf = sess.graph.get_tensor_by_name('loss:0')
        # Get the actual global step variable/tensor by trying to read the variable directly
        try:
            global_step_var = sess.graph.get_tensor_by_name('global_step/Read/ReadVariableOp:0')
        except:
            global_step_var = sess.graph.get_tensor_by_name('global_step:0')
        feed_dict = { X_tf:X, y_tf: y}

        for i in range(iterations):
            _, loss = sess.run([training_op_tf, loss_tf], feed_dict=feed_dict)
            g_step_val = sess.run(global_step_var)
            if isinstance(g_step_val, np.ndarray):
                g_step_val = int(g_step_val)
            if g_step_val % 500 == 0:
                print('training neural network: global_step={}, loss={}'.format(g_step_val, loss))
        g_step_val = sess.run(global_step_var)
        if isinstance(g_step_val, np.ndarray):
            g_step_val = int(g_step_val)
        saved_path = saver.save(sess, nn_predictor.CHECK_POINTS_NAME,
            global_step=g_step_val, write_meta_graph=False)
...
class model_two_hidden():
    def __init__(self, size_x=9, size_y=12):
        ...
        with self.graph.as_default():
            self.X = tf.compat.v1.placeholder(tf.float32, [None, size_x], name='X')
            self.y = tf.compat.v1.placeholder(tf.float32, [None, size_y], name='y')

            weight1 = tf.Variable(tf.random.uniform([size_x, size_hidden_1]), name='weight1')
            bias1 = tf.Variable(tf.zeros(size_hidden_1), name='bias1')
            activation1 = tf.nn.relu(tf.matmul(self.X, weight1) + bias1, name='activation1')

            weight2 = tf.Variable(tf.random.uniform([size_hidden_1, size_hidden_2]), name='weight2')
            bias2 = tf.Variable(tf.zeros(size_hidden_2), name='bias2')
            activation2 = tf.nn.relu(tf.matmul(activation1, weight2) + bias2, name='activation2')

            weight3 = tf.Variable(tf.random.uniform([size_hidden_2, size_y]), name='weight3')
            bias3 = tf.Variable(tf.zeros(size_y), name='bias3')
            activation3 = tf.nn.relu(tf.matmul(activation2, weight3) + bias3, name='activation3')

            self.loss = tf.reduce_mean(tf.math.squared_difference(activation3, self.y), name='loss')

            self.predicted_y = activation3
            self.global_step = tf.Variable(0, name='global_step', trainable=False)

            optimizer = tf.compat.v1.train.AdamOptimizer(name='adam_optimizer')
