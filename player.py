    ...
    def turn(self, board):
        if board.empty():
            ...
            if self.temperature > 1e-6:
                ...
                temp_adjusted_prob[np.isnan(temp_adjusted_prob)] = 1e-6
            else:
            ...
            prob_sum = sum(temp_adjusted_prob)
            if prob_sum > 0 and not np.isnan(prob_sum):
                temp_adjusted_prob /= prob_sum
            else:
                # If all probabilities are invalid, use uniform distribution
                temp_adjusted_prob = np.ones(len(puct)) / len(puct)
