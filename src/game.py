...
def board_to_canonical_3d(board_flat, player):
    ...
    Args:
        board_flat: Flat array of 81 values (from game.state)
        player: Current player (1 or -1)

    Returns:
        3-plane numpy array of shape (3, 9, 9):
        - Plane 0: Current player positions
        - Plane 1: Opponent positions
        - Plane 2: Empty positions
    """
    # Reshape to 2D
    board_2d = np.array(board_flat).reshape(9, 9)

    # Make canonical (current player is always +1)
    canonical = board_2d * player

    # Create 3 planes
    planes = np.zeros((3, 9, 9), dtype=np.float32)
...
class TicTacToe:
    def __init__(self):
        ...
        self.state = np.zeros(81) # initial state is all zeros (9x9 board)

    def get_valid_moves(self,state):
        valid_actions = np.zeros(81)
    ...
    def win_or_draw(self,state):
        ...
        for i in range(9):
            for j in range(5):  # positions 0-4, 1-5, 2-6, 3-7, 4-8
                row_sum = state[i, j:j+5].sum()
                if row_sum == 5:
                    return 1
                if row_sum == -5:
                    return -1
        ...
        for j in range(9):
            for i in range(5):  # positions 0-4, 1-5, 2-6, 3-7, 4-8
                col_sum = state[i:i+5, j].sum()
                if col_sum == 5:
                    return 1
                if col_sum == -5:
                    return -1
        ...
        for i in range(5):  # start row: 0, 1, 2, 3, 4
            for j in range(5):  # start col: 0, 1, 2, 3, 4
                diag_sum = sum(state[i+k, j+k] for k in range(5))
                if diag_sum == 5:
                    return 1
                if diag_sum == -5:
                    return -1
        ...
        for i in range(5):  # start row: 0, 1, 2, 3, 4
            for j in range(4, 9):  # start col: 4, 5, 6, 7, 8
                anti_diag_sum = sum(state[i+k, j-k] for k in range(5))
                if anti_diag_sum == 5:
                    return 1
                if anti_diag_sum == -5:
                    return -1
